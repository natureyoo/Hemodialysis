...
def parse_arg():
    parser = argparse.ArgumentParser(
        description='Prediction Blood Pressure during Hemodialysis using Deep Learning model')
...
def rnn_classification(args):
    ...
    if args.model_type == 'rnn':
        ...
        model = RNN_V3(input_fix_size, input_seq_size, hidden_size, num_layers, output_size, batch_size, dropout_rate,
                       num_class1, num_class2, num_class3).to(device)
    else:
        print('model err')
        assert ()
    ...
    seq_idx = [5, 6, 11, 12, 13, 14, 15, 16] + [i + len(full_idx) for i in range(-12, 0)]  # add HD_ntime_target
    ...
    for epoch in range(args.init_epoch, num_epochs):
        ...
        if epoch >= 0:
            utils.eval_rnn_classification_composite(val_loader, model, device, output_size, criterion, threshold,
        ...
        running_loss, running_loss_curr, running_loss_init, running_loss_under90 = 0, 0, 0, 0

        total = 0
        train_total, train_correct_init, train_correct_curr, train_correct_under_90 = 0, 0, 0, 0
        for batch_idx, ((inputs_fix, inputs_seq), (targets, composite_targets), seq_len) in enumerate(train_loader):
            ...
            loss_init = BCE_loss_with_logit(flattened_output[:, 0], flattened_target[:,0])  # 이 loss는 알아서 input에 sigmoid를 씌워줌. 그래서 input : """logit""" / 단, target : 0 or 1
            loss_curr = BCE_loss_with_logit(flattened_output[:, 1], flattened_target[:,1])
            loss_under90 = BCE_loss_with_logit(flattened_output[:, 2], flattened_target[:,2])
            loss = loss_init + loss_curr + loss_under90

            running_loss_init = loss_init.item() * (1. / (batch_idx + 1.)) + running_loss_init * (
                        batch_idx / (batch_idx + 1.))
            running_loss_curr = loss_curr.item() * (1. / (batch_idx + 1.)) + running_loss_curr * (
                        batch_idx / (batch_idx + 1.))
            ...
            train_correct_init += (pred0 == flattened_target[:, 0].long()).sum().item()
            train_correct_curr += (pred1 == flattened_target[:, 1].long()).sum().item()
            ...
            if epoch < 5:  # 5 epoch 까지는 실시간으로 loss & acc를 보겠다.
                sys.stdout.write('\r')
                sys.stdout.write(
                    '| Epoch [{}/{}], Step [{}/{}], INITIAL l: {:.4f}  CURRENT_l: {:.4f} Under90 l:{:.4f} \t INITIAL acc.: {:.4f} CURRENT acc.: {:.4f} 90 acc.: {:.4f}'
                    .format(epoch, num_epochs, batch_idx + 1, total_step, \
                            running_loss_init, running_loss_curr, running_loss_under90,
                            train_correct_init / train_total,
                            train_correct_curr / train_total, train_correct_under_90 / train_total))
