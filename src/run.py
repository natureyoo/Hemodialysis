...
def rnn_classification(args):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, targets, seq_len) in enumerate(train_loader):
            ...
            if (i + 1) % 1000 == 0:
                print('Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}'.format(epoch + 13, num_epochs, i + 1, total_step, running_loss/total))
                ...
                if best_loss > val_running_loss:
                    ...
                    torch.save(state, log_dir+'/epoch{}_iter{}_loss{:.4f}.model'.format(epoch+13, (i+1) + (total_step) * (epoch), val_running_loss))

                    sbp_confusion_matrix, sbp_log = utils.confusion_matrix(pred1, flattened_target[:, 0], num_class1)
                    dbp_confusion_matrix, dbp_log = utils.confusion_matrix(pred2, flattened_target[:, 1], num_class2)
                    utils.confusion_matrix_save_as_img(sbp_confusion_matrix.detach().cpu().numpy(), log_dir, epoch + 13, i + 1, 'val', 'sbp')
                    utils.confusion_matrix_save_as_img(dbp_confusion_matrix.detach().cpu().numpy(), log_dir, epoch + 13, i + 1, 'val', 'dbp')
